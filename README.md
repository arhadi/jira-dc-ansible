...
- **Managed nodes**: **RHEL 9 only** — `precheck.yml` hard-fails on any other
  distro/version.
- A PostgreSQL database already reachable from the Jira host (see the
...
- The Jira installer `.tar.gz` placed in `files/` before running (see below).

## Preparing the Installer

The role does **not** download Jira for you. Place the official Data Center
tarball in `files/` on the control node, named to match `jira_archive` in
`inventory/group_vars/jira.yml`, e.g.:

```
jira-dc-ansible/files/atlassian-jira-software-11.3.8.tar.gz
```

`install.yml` looks for it at `{{ playbook_dir }}/../files/{{ jira_archive }}`
and fails fast if it's missing, then copies it to `/tmp` on the target,
extracts it under `/app`, and renames the extracted folder
(`jira_extract_dir`) to `jira_install_dir`.

## Inventory

`inventory/hosts.yml` should define a `jira` group (the install playbook
targets `hosts: jira`):
  ...
  children:
    jira:
      hosts:
        ...
        # jira02.example.com:            # additional node for cluster mode
